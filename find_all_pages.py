...
# Fetch header which may have navigation
resp = opener.open(base + "/te_header.asp", timeout=5)
content = resp.read().decode("latin-1", errors="replace")
with open(r"C:\Users\rmonz\Desktop\wol-bot\te_header.html", "w", encoding="latin-1") as f:
    f.write(content)
print(f"te_header.asp: {len(content)} bytes")

# Find all links/pages
pages = re.findall(r'(te_\w+\.asp)', content, re.I)
print(f"Pages found: {set(pages)}")

# Also check for hrefs
